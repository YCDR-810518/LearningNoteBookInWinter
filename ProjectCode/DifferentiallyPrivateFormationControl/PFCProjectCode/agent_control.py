...
"""
def simulate_formation_with_attack(W, target_coords, config, u_attack=0.1, steps=500, dt=0.05):
    N = config.N
    # 加载初始位置
    with open("initial_position.json", "r") as f:
        data = json.load(f)
...
def run_compare_task(edges):
    u_attack_list = np.linspace(0, 2.0, 5)  # 用于画关系曲线
    epsilon_scales = [0.1, 0.5, 1.0, 10.0]  # 不同的隐私预算

    plt.figure(figsize=(10, 6))

    for eps_val in epsilon_scales:
        # 修改隐私上限（题目要求在不同 epsilon 下）
        current_eps_max = np.ones(10) * eps_val

        # 重新运行优化，得到该隐私水平下的最优权重 W
        config = pfc.SimulationConfig(e_R=16.0, B=6.0, epsilon_max=current_eps_max)
        ...
        plt.plot(u_attack_list, ess_values, 'o-', label=f'eps={eps_val}')

        # 3. 输出动图 (只选其中一个 u_attack 展示，否则文件太多)
        if eps_val in [0.1, 1.0]:
            traj, _ = simulate_formation_with_attack(W_opt, circle_targets, config, u_attack=1.0)
            # 这里调用你之前的 animate_formation，记得改个文件名防止覆盖
            animate_formation(traj, W_opt, config, filename=f'attack_eps_control_{eps_val}.mp4')

    plt.title("Steady-state Error $e_{ss}$ vs Attack Intensity $u_{attack}$")
    plt.xlabel("$u_{attack}$")
    plt.ylabel("$e_{ss}$")
    plt.legend()
    plt.grid(True)
    plt.savefig("analysis_curve.jpg")
    plt.show()
